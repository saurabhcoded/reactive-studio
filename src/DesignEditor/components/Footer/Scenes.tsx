...
const Scenes = () => {
  const { scenes } = useGraphicContext();
  ...
  const { contextMenuTimelineRequest } = useGraphicContext();
  ...
  return (
    <DndContext modifiers={[restrictToFirstScrollableAncestor, restrictToHorizontalAxis]} sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd} onDragStart={handleDragStart}>
      <Block id="TimelineItemsContainer" style={{ padding: "0.25rem 0.75rem", background: "#ffffff", position: "relative" }}>
        {/* Make It Display Flex If You want to show Scenes */}
        <div className={css({ display: "none", alignItems: "center" })}>
          ...
          <SortableContext items={scenes} strategy={horizontalListSortingStrategy}>
            {scenes.map((page, index) => (
              <SceneItem key={index} isCurrentScene={page.id === currentScene?.id} scene={page} index={index} changePage={changePage} preview={currentPreview && page.id === currentScene?.id ? currentPreview : page.preview ? page.preview : ""} />
            ))}
            <div
              style={{
                background: "#ffffff",
